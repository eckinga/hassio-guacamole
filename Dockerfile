...
RUN if [ "$BUILD_ARCH" = "amd64" ]; then \
  ...
  curl -SLO "https://github.com/just-containers/s6-overlay/releases/download/v1.20.0.0/s6-overlay-armhf.tar.gz" \
  && tar -xzf s6-overlay-armhf.tar.gz -C / \
  && tar -xzf s6-overlay-armhf.tar.gz -C /usr ./bin \
  && rm -rf s6-overlay-armhf.tar.gz \
  && mkdir -p ${GUACAMOLE_HOME} \
    ${GUACAMOLE_HOME}/lib \
    ${GUACAMOLE_HOME}/extensions; \
  fi
